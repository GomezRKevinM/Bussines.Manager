<?php
	...
	$ins_venta = new serviceController();

	$datos_venta=$ins_venta->seleccionarDatos("Normal","servicios INNER JOIN cliente ON servicios.cliente_id=cliente.cliente_id INNER JOIN usuario ON servicios.usuario_id=usuario.usuario_id INNER JOIN caja ON servicios.caja_id=caja.caja_id WHERE (venta_codigo='$code')","*",0);
	...
	if($datos_venta->rowCount()==1){
        ...
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","Caja Nr°: ".$datos_venta['caja_numero']),0,'C',false);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","Usuario: ".$datos_venta['usuario_nombre']." ".$datos_venta['usuario_apellido']),0,'C',false);
        $pdf->SetFont('Arial','B',10);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1",strtoupper("Ticket de servicio Nro: ".$datos_venta['venta_id'])),0,'C',false);
        $pdf->SetFont('Arial','',9);
		...
		$venta_detalle=$ins_venta->seleccionarDatos("Normal","servicios_detalle WHERE venta_codigo='".$datos_venta['venta_codigo']."'","*",0);
        ...
        $pdf->Cell(0,7,iconv("UTF-8", "ISO-8859-1","Gracias por confiar en nosotros"),'',0,'C');
    ...
    <div class="main-container">
        <section class="hero-body">
            <div class="hero-body">
                ...
                <p class="subtitle has-text-white">No hemos encontrado datos del servicio</p>
            </div>
        </section>
    </div>
