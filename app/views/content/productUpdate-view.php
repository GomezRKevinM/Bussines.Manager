		    	...
		    	<div class="control">
					<label>Presentación del producto <?php echo CAMPO_OBLIGATORIO; ?></label><br>
					  ...
					  <select name="producto_unidad" >
				    	<?php
                            $datos_categorias=$insLogin->seleccionarDatos("Normal","presentacion","*",0);

                            $cc=1;
                            while($campos_categoria=$datos_categorias->fetch()){
                            	if($campos_categoria['presentacion_nombre']==$datos['presentacion_nombre']){
                            		echo '<option value="'.$campos_categoria['presentacion_nombre'].'" selected="" >'.$cc.' - '.$campos_categoria['presentacion_nombre'].' (Actual)</option>';
                            	}else{
