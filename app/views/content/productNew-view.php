...
<div class="container is-fluid">
	...
	<form class="FormularioAjax" action="<?php echo APP_URL; ?>app/ajax/productoAjax.php" method="POST" autocomplete="off" enctype="multipart/form-data" >
					...
					<label>Presentación del producto <?php echo CAMPO_OBLIGATORIO; ?></label><br>
				  	<div class="select">
					  	<select name="producto_unidad">
	                        ...
	                        <?php
	                        	 $datos_categorias=$insLogin->seleccionarDatos("Normal","presentacion WHERE company_id=".$_SESSION['company'],"*",0);

								 $cc=1;
								 while($campos_categoria=$datos_categorias->fetch()){
									 echo '<option value="'.$campos_categoria['presentacion_nombre'].'">'.$cc.' - '.$campos_categoria['presentacion_nombre'].'</option>';
									 $cc++;
								 }
	                        ?>
					  	</select>
		...
		<div class="column">
		    	<div class="control">
					<label>Fecha de expiración <strong>(opcional)</strong></label><br>
				  	<input class="input" style="width: 150px;" type="date" name="producto_vence" pattern="[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ().,#\- ]{1,30}" maxlength="30" >
				</div>
		  	</div>
			<div class="column">
