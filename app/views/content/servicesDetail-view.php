<div class="container is-fluid mb-6">
	<h1 class="title">Servicios Realizados</h1>
	<h2 class="subtitle"><i class="fas fa-shopping-bag fa-fw"></i> &nbsp; Información de Servicio</h2>
</div>
...
<div class="container is-fluid">
	<?php
		...
		$datos=$insLogin->seleccionarDatos("Normal","servicios INNER JOIN cliente ON servicios.cliente_id=cliente.cliente_id INNER JOIN usuario ON servicios.usuario_id=usuario.usuario_id INNER JOIN caja ON servicios.caja_id=caja.caja_id WHERE (venta_codigo='".$code."')","*",0);

		if($datos->rowCount()==1){
			$datos_venta=$datos->fetch();
	?>
	<h2 class="title has-text-centered">Datos del servicio <?php echo " (".$code.")"; ?></h2>
	<div class="columns pb-6 pt-6">
		<div class="column">
			...
			<div class="full-width sale-details text-condensedLight">
				<div class="has-text-weight-bold">Nro. de Servicio</div>
				<span class="has-text-link"><?php echo $datos_venta['venta_id']; ?></span>
			</div>

			<div class="full-width sale-details text-condensedLight">
				<div class="has-text-weight-bold">Código de Servicio</div>
		...
		<div class="column">
			...
			<div class="full-width sale-details text-condensedLight">
				<div class="has-text-weight-bold">Tecnico</div>
		...
		<div class="column">
			<div class="table-container">
                <table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
                    ...
                    <tbody>
                        <?php
                        	$detalle_venta=$insLogin->seleccionarDatos("Normal","servicios_detalle WHERE venta_codigo='".$datos_venta['venta_codigo']."'","*",0);
	...
	<div class="columns pb-6 pt-6">
		<p class="has-text-centered full-width">
			<?php
			echo '<button type="button" class="button is-link is-light is-medium" onclick="print_invoiceS(\''.APP_URL.'app/pdf/invoiceS.php?code='.$datos_venta['venta_codigo'].'\')" title="Imprimir factura de Servicio Nro. '.$datos_venta['venta_id'].'" >
			<i class="fas fa-file-invoice-dollar fa-fw"></i> &nbsp; Imprimir factura de Servicio
			</button> &nbsp;&nbsp; 

			<button type="button" class="button is-link is-light is-medium" onclick="print_ticketS(\''.APP_URL.'app/pdf/ticketS.php?code='.$datos_venta['venta_codigo'].'\')" title="Imprimir ticket de Servicio Nro. '.$datos_venta['venta_id'].'" ><i class="fas fa-receipt fa-fw"></i> &nbsp; Imprimir ticket de Servicio</button>';
			?>
		</p>
	</div>
	<?php
