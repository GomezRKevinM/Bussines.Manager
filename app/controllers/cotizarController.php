<?php
	...
	class cotizarController extends mainModel{
        ...
        public function buscarCodigoVentaControlador(){
            ...
            if($datos_productos->rowCount()>=1){
				...
				$datos_servicios=$datos_servicios->fetchAll();

				$tabla='<div class="table-container mb-6"><table class="table is-striped is-narrow is-hoverable is-fullwidth"><tbody>';

				foreach($datos_servicios as $rows){
					$tabla.='
					<tr class="has-text-left" >
        ...
        /*---------- Controlador agregar producto a cotización ----------*/
        public function agregarProductoCarritoControlador(){
            ...
            if(empty($_SESSION['datos_producto_venta'][$codigo])){
                ...
                $_SESSION['alerta_producto_agregado']="Se agrego <strong>".$campos['producto_nombre']."</strong> a la cotización";
            }else{
                ...
                if($stock_total<0){
                    $alerta=[
						"tipo"=>"simple",
						"titulo"=>"Advertencia",
						"texto"=>"Actualmente no hay existencias disponibles del producto seleccionado",
						"icono"=>"error"
					];
					return json_encode($alerta);
                }
                ...
                $_SESSION['alerta_producto_agregado']="Se agrego +1 <strong>".$campos['producto_nombre']."</strong> a la cotización. Total en carrito: <strong>$detalle_cantidad</strong>";
            }

            $alerta=[
				"tipo"=>"redireccionar",
				"url"=>APP_URL."cotizacionNew/"
			];
        ...
        /*---------- Controlador remover producto de cotización ----------*/
        public function removerProductoCarritoControlador(){
            ...
            if(empty($_SESSION['datos_producto_venta'][$codigo])){
				$alerta=[
					...
					"texto"=>"El producto se ha removido de la cotización",
					"icono"=>"success"
				];
        ...
        /*---------- Controlador actualizar producto de cotización ----------*/
        public function actualizarProductoCarritoControlador(){
            ...
            if(!empty($_SESSION['datos_producto_venta'][$codigo])){
                ...
                $_SESSION['alerta_producto_agregado']="Se $diferencia_productos <strong>".$campos['producto_nombre']."</strong> a la cotización. Total en carrito <strong>$detalle_cantidad</strong>";
        ...
        public function agregarClienteVentaControlador(){
			...
			if($_SESSION['datos_cliente_venta']['cliente_id']==1){
				...
				$alerta=[
					...
					"texto"=>"El cliente se agregó para realizar una cotización",
					"icono"=>"success"
				];
			}else{
				$alerta=[
        ...
        public function removerClienteVentaControlador(){
			...
			if(empty($_SESSION['datos_cliente_venta'])){
				$alerta=[
					...
					"texto"=>"Los datos del cliente se han quitado de la cotización",
					"icono"=>"success"
				];
			}else{
				$alerta=[
        ...
        /*---------- Controlador registrar cotización ----------*/
        public function registrarVentaControlador(){
            ...
            if($_SESSION['venta_total']<=0 || (!isset($_SESSION['datos_producto_venta']) && count($_SESSION['datos_producto_venta'])<=0)){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No ha agregado productos a esta cotización",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if(!isset($_SESSION['datos_cliente_venta'])){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No ha seleccionado ningún cliente para realizar esta cotización",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            $movimiento_cantidad=$venta_pagado;
			...
			foreach($_SESSION['datos_producto_venta'] as $productos){
                ...
                $_SESSION['datos_producto_venta'][$productos['producto_codigo']]['producto_stock_total']=$datos_producto['producto_stock_total'];
            ...
            /*== Agregando venta ==*/
            $agregar_venta=$this->guardarDatos("cotizaciones",$datos_venta_reg);
            ...
            if($agregar_venta->rowCount()!=1){
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido registrar la cotizacion, por favor intente nuevamente. Código de error: 001",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            /*== Agregando detalles de la cotización ==*/
            ...
            foreach($_SESSION['datos_producto_venta'] as $venta_detalle){
                ...
                $datos_venta_detalle_reg=[
					...
					[
						"campo_nombre"=>"cotizacion_detalle_precio",
						"campo_marcador"=>":PrecioCotizacion",
					...
					[
						"campo_nombre"=>"cotizacion_detalle_detalles",
						"campo_marcador"=>":DetallesComment",
						"campo_valor"=>$detail
					],
					[
						"campo_nombre"=>"cotizacion_codigo",
						"campo_marcador"=>":CotizacionCodigo",
						"campo_valor"=>$codigo_venta
					],
					[
                ...
                $agregar_detalle_venta=$this->guardarDatos("cotizacion_detalle",$datos_venta_detalle_reg);
            ...
            if($errores_venta_detalle==1){
                ...
                $this->eliminarRegistro("cotizacion_detalle","cotizacion_codigo",$codigo_venta);
                $this->eliminarRegistro("cotizaciones","cotizacion_codigo",$codigo_venta);
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido registrar la cotizacion, por favor intente nuevamente. Código de error: 002",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if(!$this->actualizarDatos("caja",$datos_caja_up,$condicion_caja)){
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido registrar la cotización, por favor intente nuevamente. Código de error: 003",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            $alerta=[
				...
				"texto"=>"La cotización se registró con éxito en el sistema",
				"icono"=>"success"
			];
			return json_encode($alerta);
	        exit();
		...
		public function listarVentaDateControlador($pagina,$registros,$url,$busqueda,$fechaF){
			...
			$campos_tablas="cotizaciones.cotizacion_id,cotizaciones.cotizacion_codigo,cotizaciones.cotizacion_fecha,cotizaciones.cotizacion_hora,cotizaciones.cotizacion_total,cotizaciones.usuario_id,cotizaciones.cliente_id,cotizaciones.caja_id,usuario.usuario_id,usuario.usuario_nombre,usuario.usuario_apellido,cliente.cliente_id,cliente.cliente_nombre,cliente.cliente_apellido";
			...
			$tabla.='
				<div><i class="bi bi-bag-check-fill"></i> &nbsp;Cotizaciones Realizadas en este periodo: <strong>'.$totalDatos.'</strong><div>
				...
				foreach($datos as $rows){
					$tabla.='
						<tr class="has-text-centered" >
							<td>'.$rows['cotizacion_id'].'</td>
							<td>'.$rows['cotizacion_codigo'].'</td>
							<td>'.date("d-m-Y", strtotime($rows['cotizacion_fecha'])).' '.$rows['cotizacion_hora'].'</td>
							...
							<td>'.MONEDA_SIMBOLO.number_format($rows['cotizacion_total']).' '.MONEDA_NOMBRE.'</td>
							<td>

								<button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_invoice(\''.APP_URL.'app/pdf/invoiceC.php?code='.$rows['cotizacion_codigo'].'\')" title="Imprimir factura Nro. '.$rows['cotizacion_id'].'" >
									<i class="fas fa-file-invoice-dollar fa-fw"></i>
								</button>

								<button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_ticket(\''.APP_URL.'app/pdf/ticketC.php?code='.$rows['cotizacion_codigo'].'\')" title="Imprimir ticket Nro. '.$rows['cotizacion_id'].'" >
									<i class="fas fa-receipt fa-fw"></i>
								</button>

								<a href="'.APP_URL.'cotizacionDetail/'.$rows['cotizacion_codigo'].'/" class="button is-link is-rounded is-small" title="Informacion de cotización Nro. '.$rows['cotizacion_id'].'" >
									<i class="fas fa-shopping-bag fa-fw"></i>
								</a>

								<form class="FormularioAjax is-inline-block" action="'.APP_URL.'app/ajax/cotizacionAjax.php" method="POST" autocomplete="off" >

									<input type="hidden" name="modulo_venta" value="eliminar_venta">
									<input type="hidden" name="venta_id" value="'.$rows['cotizacion_id'].'">

									<button type="submit" class="button is-danger is-rounded is-small" title="Eliminar cotización Nro. '.$rows['cotizacion_id'].'" >
					...
					$tabla.='
						<tr class="has-text-centered" >
			                ...
			                <td>
			                	...
			                	<button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_invoiceCot(\''.APP_URL.'app/pdf/invoiceC.php?code='.$rows['cotizacion_codigo'].'\')" title="Imprimir factura Nro. '.$rows['cotizacion_id'].'" >
	                                <i class="fas fa-file-invoice-dollar fa-fw"></i>
	                            </button>

                                <button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_ticketCot(\''.APP_URL.'app/pdf/ticketC.php?code='.$rows['cotizacion_codigo'].'\')" title="Imprimir ticket Nro. '.$rows['cotizacion_id'].'" >
                                    <i class="fas fa-receipt fa-fw"></i>
                                </button>
		...
		public function eliminarVentaControlador(){
		    ...
		    if($datos->rowCount()<=0){
		        $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos encontrado la Cotización en el sistema",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
		    ...
		    if($check_detalle_venta>0){
		        ...
		        $eliminarVentaDetalle=$this->eliminarRegistro("cotizacion_detalle","cotizacion_codigo",$datos['cotizacion_codigo']);

		        if($eliminarVentaDetalle->rowCount()!=$check_detalle_venta){
		        	$alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
						"texto"=>"No hemos podido eliminar la cotización del sistema, por favor intente nuevamente",
						"icono"=>"error"
					];
					return json_encode($alerta);
			        exit();
		    ...
		    $eliminarVenta=$this->eliminarRegistro("cotizaciones","cotizacion_id",$id);
		    ...
		    if($eliminarVenta->rowCount()==1){
		        ...
		        $alerta=[
					...
					"texto"=>"La cotización ha sido eliminada del sistema correctamente",
					"icono"=>"success"
				];
