<?php
	...
	class saleController extends mainModel{
        ...
        public function buscarCodigoVentaControlador(){
            ...
            $datos_productos=$this->ejecutarConsulta("SELECT * FROM producto WHERE company_id=".$_SESSION['company']." AND (producto_nombre LIKE '%$producto%' OR producto_marca LIKE '%$producto%' OR producto_modelo LIKE '%$producto%') ORDER BY producto_nombre ASC");
        ...
        public function agregarProductoCarritoControlador(){
            ...
            $check_producto=$this->ejecutarConsulta("SELECT * FROM producto WHERE company_id=".$_SESSION['company']." AND producto_codigo='$codigo'");
        ...
        public function buscarClienteVentaControlador(){
            ...
            $datos_cliente=$this->ejecutarConsulta("SELECT * FROM cliente WHERE (cliente_id!='1' AND company_id=".$_SESSION['company'].") AND (cliente_numero_documento LIKE '%$cliente%' OR cliente_nombre LIKE '%$cliente%' OR cliente_apellido LIKE '%$cliente%' OR cliente_telefono LIKE '%$cliente%') ORDER BY cliente_nombre ASC");
        ...
        public function agregarClienteVentaControlador(){
			...
			$check_cliente=$this->ejecutarConsulta("SELECT * FROM cliente WHERE  company_id=".$_SESSION['company']." AND cliente_id='$id'");
        ...
        public function registrarVentaControlador(){
			...
			foreach($_SESSION['datos_producto_venta'] as $productos){
                ...
                $check_producto=$this->ejecutarConsulta("SELECT * FROM producto WHERE company_id=".$_SESSION['company']." AND producto_id='".$productos['producto_id']."' AND producto_codigo='".$productos['producto_codigo']."'");
            ...
            $correlativo=$this->ejecutarConsulta("SELECT venta_id FROM venta WHERE company_id=".$_SESSION['company']);
		...
		public function listarVentaDateControlador($pagina,$registros,$url,$busqueda,$fechaF){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE venta.company_id=".$empresa." AND venta_fecha BETWEEN '$busqueda' AND '$fechaF' ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(venta_id) FROM venta WHERE venta.company_id=".$empresa." AND venta_fecha";

			}else{

				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE venta.company_id=".$empresa." ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(venta_id) FROM venta WHERE venta.company_id=".$empresa;
			...
			$tabla.='
				<div><i class="bi bi-bag-check-fill"></i> &nbsp;Ventas Realizadas en este periodo: <strong>'.$totalDatos.'</strong><div>
				<div class="table-container">
				<table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
					<thead>
						<tr>
							<th class="has-text-centered is-primary">NRO.</th>
		...
		public function listarVentaControlador($pagina,$registros,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE venta.company_id=".$empresa." AND (venta.venta_codigo='$busqueda') ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(venta_id) FROM venta WHERE company_id=".$empresa." AND (venta.venta_codigo='$busqueda')";

			}else{

				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE venta.company_id=".$empresa." ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(venta_id) FROM venta WHERE company_id=".$empresa;
			...
			$tabla.='
		        ...
		        <table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
		            <thead>
		                <tr class="is-primary">
		                    <th class="has-text-centered has-text-white">NRO.</th>
		                    <th class="has-text-centered has-text-white">Codigo</th>
		                    <th class="has-text-centered has-text-white">Fecha</th>
		                    <th class="has-text-centered has-text-white">Cliente</th>
		                    <th class="has-text-centered has-text-white">Vendedor</th>
		...
		/*----------  Controlador listar venta por cliente  ----------*/
		public function listarVentaClientControlador($pagina,$registros,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT $campos_tablas FROM venta INNER JOIN cliente ON venta.cliente_id=cliente.cliente_id INNER JOIN usuario ON venta.usuario_id=usuario.usuario_id WHERE venta.company_id=".$empresa." AND (venta.cliente_id='$busqueda') ORDER BY venta.venta_id DESC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT('venta_id') FROM venta WHERE venta.company_id=".$empresa." AND cliente_id='$busqueda'";
			...
			$tabla.='
				...
				<table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
					<thead>
						<tr>
							<th class="has-text-centered is-black">NRO.</th>
							<th class="has-text-centered is-success">Codigo</th>
							<th class="has-text-centered is-success">Fecha</th>
							<th class="has-text-centered is-success">Cliente</th>
							<th class="has-text-centered is-success">Diferencia</th>
							<th class="has-text-centered is-success">Vendedor</th>
							<th class="has-text-centered is-success	">Total</th>
							<th class="has-text-centered is-success">Opciones</th>
						</tr>
					</thead>
					<tbody>
			';
			...
			if($total>=1 && $pagina<=$numeroPaginas){
				...
				foreach($datos as $rows){
					...
					$totalVentas = $cliente[0]['cliente_ventas'];
					$fechaCliente = new \DateTime($rows['venta_fecha']);
					$dias = $fechaCliente ->format('d');
					$mes = $fechaCliente ->format('m');
					$año = $fechaCliente ->format('Y');
					$datosDias = [$dias];
					$diferencia = 
					
					$tabla.='
						<tr class="has-text-centered" >
							<td class="is-dark">'.$contador.'</td>
							<td>'.$rows['venta_codigo'].'</td>
							<td>'.date("d-m-Y", strtotime($rows['venta_fecha'])).' '.$rows['venta_hora'].'</td>
							<td>'.$this->limitarCadena($rows['cliente_nombre'].' '.$rows['cliente_apellido'],30,"...").'</td>
							<td>'.$año.'</td>
				...
				if($total>=1){
					...
					$tabla.='
						<tr class="has-text-centered" >
							<td class="is-info" colspan="7">
								No hay registro de compras del cliente en el sistema
							</td>
						</tr>
					';
				}
			...
			$tabla.='<tr><th class="is-black has-text-centered" colspan="8">COMPRAS REALIZADAS <span class="num_container">'.$totalVentas.'<span></th></tr></tbody></table></div>';
		...
		public function eliminarVentaControlador(){
			...
			if($_SESSION['rol']=="Empleado" || $_SESSION['rol']=="Cajero"){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ups... Error",
					"texto"=>"Solo el Administrador puede eliminar ventas",
					"icono"=>"error"
				];
				return json_encode($alerta);
				exit();
