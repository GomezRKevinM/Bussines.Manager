<?php
	...
	class productController extends mainModel{
		...
		public function registrarProductoControlador(){
			...
			$expria=$this->limpiarCadena($_POST['producto_vence']);
		    ...
		    $check_codigo=$this->ejecutarConsulta("SELECT producto_codigo FROM producto WHERE company_id=".$empresa." AND producto_codigo='$codigo'");
		    ...
		    $check_nombre=$this->ejecutarConsulta("SELECT producto_nombre FROM producto WHERE company_id=".$empresa." AND producto_codigo='$codigo' AND producto_nombre='$nombre'");
		        ...
		        if(!move_uploaded_file($_FILES['producto_foto']['tmp_name'],$img_dir.$foto)){
					...
					if($expria==""){
						$producto_datos_reg=[
							...
							[
								"campo_nombre"=>"producto_vence",
								"campo_marcador"=>":Vence",
								"campo_valor"=>""
							],
							[
		...
		/*----------  Controlador listar producto categoria  ----------*/
		public function listarProductoCategoryControlador($pagina,$registros,$url,$busqueda,$categoria){
			...
			$campos="producto.producto_vence,producto.producto_id,producto.producto_codigo,producto.producto_nombre,producto_stock_total,producto.producto_precio_venta,producto.producto_foto,producto.company_id,categoria.categoria_nombre,categoria.company_id";

			if(isset($busqueda) && $busqueda!=""){

				$consulta_datos="SELECT $campos FROM producto WHERE company_id=".$empresa." INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%' ORDER BY producto_nombre ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(producto_id) FROM producto WHERE (company_id=".$empresa.") AND (producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%')";
		        ...
		        $consulta_total="SELECT COUNT(producto_id) FROM producto WHERE company_id=".$empresa." AND categoria_id='$categoria'";

				$consulta_stock_total="SELECT SUM(producto_stock_total) FROM producto WHERE company_id=".$empresa." AND categoria_id='$categoria' ";

		    }else{

				$consulta_datos="SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE company_id=".$empresa." ORDER BY producto_nombre ASC LIMIT $inicio,$registros";
			...
			$totalStock = $this->ejecutarConsulta($consulta_stock_total);
			$totalStock= (int) $totalStock->fetchColumn();
		    ...
		    if($total>=1 && $pagina<=$numeroPaginas){
				...
				foreach($datos as $rows){
					$PrecioVenta = $rows['producto_precio_venta'];
					$cantidadInStock = $rows['producto_stock_total'];
					if($rows['producto_vence']=="0000-00-00"){
						$expira="Sin Fecha De Expiración";
						$line="";
					}else{
						$expira=$rows['producto_vence'];
						$line="<strong>Vence en:</strong> $expira ";
					}
					$valorStock = $PrecioVenta * $cantidadInStock;
					$tabla.='
		            <article class="media pb-3 pt-3">
		                <figure class="media-left">
		                    <p class="image is-64x64">';
		                        if(is_file("./app/views/productos/".$rows['producto_foto'])){
		                            $tabla.='<img class="img_product" src="'.APP_URL.'app/views/productos/'.$rows['producto_foto'].'">';
		                        }else{
		                ...
		                <div class="media-content">
		                    <div class="content">
		                        <p>
		                            ...
		                            <strong>PRECIO:</strong> '.MONEDA_SIMBOLO.number_format($rows['producto_precio_venta'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL).' 
		                            '.$line.'
									<strong>STOCK: <span class="stock">'.$rows['producto_stock_total'].'</span></strong>  
		                            <strong>CATEGORIA:</strong> '.$rows['categoria_nombre'].' 
									<strong>Valor en Inventario:</strong>'.MONEDA_SIMBOLO.number_format($valorStock,MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL).'
		                        </p>
		                    </div>
			...
			if($total>0 && $pagina<=$numeroPaginas){
				...
				$tabla.='<script>
						...
						document.querySelector("#totalBar").innerHTML=totalStock;
						document.querySelector("#slowCant").innerHTML=totalSlow;
...
';
		...
		public function listarProductoControlador($pagina,$registros,$url,$busqueda,$categoria){
			...
			$campos="producto.company_id,producto.producto_vence,producto.producto_id,producto.producto_codigo,producto.producto_nombre,producto_stock_total,producto.producto_precio_venta,producto.producto_foto,categoria.categoria_nombre,categoria.company_id";

			if(isset($busqueda) && $busqueda!=""){

				$consulta_datos="SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE (producto.company_id=".$empresa.") AND (producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%') ORDER BY producto_nombre ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(producto_id) FROM producto WHERE (producto.company_id=".$empresa.") AND (producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%')";

				$consulta_stock_total="SELECT SUM(producto_stock_total) FROM producto WHERE (producto.company_id=".$empresa.") AND (producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%') ";

				$totalSlow = "SELECT COUNT(*) FROM producto WHERE (producto.company_id=".$empresa.") AND (producto_codigo LIKE '%$busqueda%' OR producto_nombre LIKE '%$busqueda%' OR producto_marca LIKE '%$busqueda%' OR producto_modelo LIKE '%$busqueda%') AND producto_stock_total BETWEEN 0 AND 10 ";

			}elseif($categoria>0){

		        $consulta_datos="SELECT $campos FROM producto WHERE company_id=".$empresa." INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE producto.categoria_id='$categoria' ORDER BY producto.producto_nombre ASC LIMIT $inicio,$registros";

		        $consulta_total="SELECT COUNT(producto_id) FROM producto WHERE company_id=".$empresa." AND categoria_id='$categoria'";

				$consulta_stock_total="SELECT SUM(producto_stock_total) FROM producto WHERE producto.company_id=".$empresa." AND categoria_id='$categoria' ";

		    }else{

				$consulta_datos="SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE producto.company_id=".$empresa."  ORDER BY producto_nombre  ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(producto_id) FROM producto WHERE producto.company_id=".$empresa;

				$consulta_stock_total="SELECT SUM(producto_stock_total) FROM producto WHERE producto.company_id=".$empresa;
				$consulta_slow_total="SELECT COUNT(*) FROM producto  WHERE producto.company_id=".$empresa." AND producto_stock_total BETWEEN 0 AND 10 ";
			

				$totalSlow= $this->ejecutarConsulta($consulta_slow_total);
				$totalSlow= (int) $totalSlow->fetchColumn();
			}
...
';
		...
		/*----------  Controlador listar producto  ----------*/
		public function listarProductoSlowControlador($pagina,$registros,$url,$busqueda,$categoria){
			...
			$campos="producto.company_id,producto.producto_vence,producto.producto_id,producto.producto_codigo,producto.producto_nombre,producto_stock_total,producto.producto_precio_venta,producto.producto_foto,categoria.categoria_nombre,categoria.company_id";
				...
				$consulta_datos="SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id WHERE producto.company_id=".$empresa." AND producto_stock_total BETWEEN 0 AND 10 ORDER BY producto_nombre ASC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(producto_id) FROM producto WHERE producto.company_id=".$empresa." AND producto_stock_total BETWEEN 0 AND 10";
		    ...
		    if($total>=1 && $pagina<=$numeroPaginas){
				...
				if($total>=1){
					...
					$tabla.='
						<p class="has-text-centered pb-6"><i class="far fa-grin-beam-sweat fa-5x"></i></p>
						<p class="has-text-centered">No hay productos con pocas unidades</p>
					';
				}
			}
		...
		public function actualizarProductoControlador(){
			...
			# Verificando Presentacion #
			if($datos['producto_tipo_unidad']!=$unidad){
			    $check_categoria=$this->ejecutarConsulta("SELECT presentacion_nombre FROM presentacion WHERE company_id=".$empresa." AND presentacion_nombre='$unidad'");
			    if($check_categoria->rowCount()<=0){
			        $alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
						"texto"=>"La presentacion seleccionada no existe en el sistema",
						"icono"=>"error"
					];
					return json_encode($alerta);
			        exit();
		...
		public function actualizarFotoProductoControlador(){
			...
			# Verificando producto #
		    $datos=$this->ejecutarConsulta("SELECT * FROM producto WHERE company_id=".$empresa." AND producto_id='$id'");
		    if($datos->rowCount()<=0){
		        $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
