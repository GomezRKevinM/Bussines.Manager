<?php
	...
	class serviceController extends mainModel{
        ...
        public function buscarCodigoVentaControlador(){
			...
			if($producto==""){
				...
				<article class="message is-warning mt-4 mb-4">
				    ...
				    <div class="message-body has-text-centered">
						...
						Debes de introducir el Nombre del servicio
				    </div>
				</article>';
				exit();
            }
            ...
            $datos_productos=$this->ejecutarConsulta("SELECT * FROM servicio WHERE (company_id=".$empresa.") AND (servicio_nombre  LIKE '%$producto%') ORDER BY servicio_nombre ASC");
            ...
            if($datos_productos->rowCount()>=1){
				...
				foreach($datos_productos as $rows){
					...
					<tr class="has-text-left" >
                        <td><i class="fas fa-box fa-fw"></i> &nbsp; '.$rows['servicio_nombre'].'</td>
                        <td class="has-text-centered">
                            <button type="button" class="button is-link is-rounded is-small" onclick="agregar_codigo(\''.$rows['servicio_codigo'].'\')"><i class="fas fa-plus-circle"></i></button>
                        </td>
                    </tr>
                    ';
				}
				...
				return '<article class="message is-warning mt-4 mb-4">
				    ...
				    <div class="message-body has-text-centered">
				    	<i class="fas fa-exclamation-triangle fa-2x"></i><br>
						No hemos encontrado ningún servicio en el sistema que coincida con <strong>“'.$producto.'”
				    </div>
				</article>';
        ...
        /*---------- Controlador agregar servicio a venta ----------*/
        public function agregarProductoCarritoControlador(){

            /*== Recuperando codigo del servicio ==*/
            ...
            if($codigo==""){
                $alerta=[
					...
					"texto"=>"Debes de introducir el código de barras del servicio",
					"icono"=>"error"
				];
            ...
            $check_producto=$this->ejecutarConsulta("SELECT * FROM servicio WHERE company_id=".$empresa." AND servicio_codigo='$codigo'");
            if($check_producto->rowCount()<=0){
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos encontrado el servicio con código de barras : '$codigo'",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            /*== Codigo de servicio ==*/
            $codigo=$campos['servicio_codigo'];

            if(empty($_SESSION['datos_producto_venta'][$codigo])){

                $detalle_cantidad=1;

                $detalle_total=$detalle_cantidad*$campos['servicio_precio'];
                $detalle_total=number_format($detalle_total,MONEDA_DECIMALES,'.','');

                $_SESSION['datos_producto_venta'][$codigo]=[
                    "producto_id"=>$campos['servicio_id'],
					"producto_codigo"=>$campos['servicio_codigo'],
                    "venta_detalle_precio_venta"=>$campos['servicio_precio'],
                    "venta_detalle_cantidad"=>1,
                    "venta_detalle_total"=>$detalle_total,
                    "venta_detalle_descripcion"=>$campos['servicio_nombre']
                ];

                $_SESSION['alerta_producto_agregado']="Se agrego <strong>".$campos['servicio_nombre']."</strong> al servicio";
            }else{
                ...
                $_SESSION['alerta_producto_agregado']="Se agrego +1 <strong>".$campos['servicio_nombre']."</strong> al servicio. Total en Ticket: <strong>$detalle_cantidad</strong>";
            }

            $alerta=[
				"tipo"=>"redireccionar",
				"url"=>APP_URL."servicesNew/"
			];
        ...
        /*---------- Controlador remover producto de servicio ----------*/
        public function removerProductoCarritoControlador(){
            ...
            if(empty($_SESSION['datos_producto_venta'][$codigo])){
				$alerta=[
					...
					"texto"=>"El servicio se ha removido de la venta",
					"icono"=>"success"
				];
				...
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido remover el servicio, por favor intente nuevamente",
					"icono"=>"error"
				];
            }
            return json_encode($alerta);
        ...
        /*---------- Controlador actualizar producto de servicio ----------*/
        public function actualizarProductoCarritoControlador(){
            ...
            if($codigo=="" || $cantidad==""){
            	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No podemos actualizar la cantidad de servicios debido a que faltan algunos parámetros de configuración",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if(!empty($_SESSION['datos_producto_venta'][$codigo])){
                ...
                if($_SESSION['datos_producto_venta'][$codigo]["venta_detalle_cantidad"]==$cantidad){
                    $alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
						"texto"=>"No has modificado la cantidad de servicios",
						"icono"=>"error"
					];
					return json_encode($alerta);
			        exit();
                ...
                $_SESSION['alerta_producto_agregado']="Se $diferencia_productos <strong>".$campos['servicio_nombre']."</strong> a la venta. Total en Ticket <strong>$detalle_cantidad</strong>";
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos encontrado el servicio que desea actualizar en el Ticket",
					"icono"=>"error"
				];
				return json_encode($alerta);
            }
        ...
        public function agregarClienteVentaControlador(){
			...
			if($_SESSION['datos_cliente_venta']['cliente_id']==1){
				...
				$alerta=[
					...
					"texto"=>"El cliente se agregó para realizar una venta de servicio",
					"icono"=>"success"
				];
			}else{
				$alerta=[
        ...
        public function removerClienteVentaControlador(){
			...
			if(empty($_SESSION['datos_cliente_venta'])){
				$alerta=[
					...
					"texto"=>"Los datos del cliente se han quitado del servicio",
					"icono"=>"success"
				];
			}else{
				$alerta=[
        ...
        public function registrarVentaControlador(){
            ...
            if($_SESSION['venta_total']<=0 || (!isset($_SESSION['datos_producto_venta']) && count($_SESSION['datos_producto_venta'])<=0)){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No ha agregado servicios a esta venta",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if(!isset($_SESSION['datos_cliente_venta'])){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No ha seleccionado ningún cliente para realizar este servicio",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
			...
			foreach($_SESSION['datos_producto_venta'] as $productos){
                ...
                $check_producto=$this->ejecutarConsulta("SELECT * FROM servicio WHERE (company_id=".$empresa.") AND (servicio_id='".$productos['producto_id']."' AND servicio_codigo='".$productos['producto_codigo']."')");
                ...
                if(!$this->actualizarDatos("servicio",$datos_producto_up,$condicion)){
                    $errores_productos=1;
                    break;
                }
            }
            ...
            if($errores_productos==1){
                ...
                foreach($_SESSION['datos_producto_venta'] as $producto){
                    ...
                    $this->actualizarDatos("servicio",$datos_producto_rs,$condicion);
                }

                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido actualizar los servicios en el sistema",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            $correlativo=$this->ejecutarConsulta("SELECT venta_id FROM servicios WHERE company_id=".$empresa);
            ...
            /*== Agregando venta ==*/
            $agregar_venta=$this->guardarDatos("servicios",$datos_venta_reg);
            ...
            if($agregar_venta->rowCount()!=1){
                foreach($_SESSION['datos_producto_venta'] as $producto){
                    ...
                    $this->actualizarDatos("servicios",$datos_producto_rs,$condicion);
                }

                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido registrar la venta del servicio, por favor intente nuevamente. Código de error: 001",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            foreach($_SESSION['datos_producto_venta'] as $venta_detalle){
                ...
                $agregar_detalle_venta=$this->guardarDatos("servicios_detalle",$datos_venta_detalle_reg);
            ...
            if($errores_venta_detalle==1){
                ...
                $this->eliminarRegistro("servicios_detalle","venta_codigo",$codigo_venta);
                $this->eliminarRegistro("servicios","venta_codigo",$codigo_venta);
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido registrar la venta del servicio, por favor intente nuevamente. Código de error: 002",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if(!$this->actualizarDatos("caja",$datos_caja_up,$condicion_caja)){
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido registrar la venta del servicio, por favor intente nuevamente. Código de error: 003",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            $alerta=[
				...
				"texto"=>"La venta del servicio se registró con éxito en el sistema",
				"icono"=>"success"
			];
			return json_encode($alerta);
	        exit();
		...
		public function listarVentaDateControlador($pagina,$registros,$url,$busqueda,$fechaF){
			...
			$campos_tablas="servicios.company_id,servicios.venta_id,servicios.venta_codigo,servicios.venta_fecha,servicios.venta_hora,servicios.venta_total,servicios.usuario_id,servicios.cliente_id,servicios.caja_id,usuario.usuario_id,usuario.usuario_nombre,usuario.usuario_apellido,cliente.cliente_id,cliente.cliente_nombre,cliente.cliente_apellido";

			if(isset($busqueda) && $busqueda!=""){

				$consulta_datos="SELECT $campos_tablas FROM servicios INNER JOIN cliente ON servicios.cliente_id=cliente.cliente_id INNER JOIN usuario ON servicios.usuario_id=usuario.usuario_id WHERE (servicios.company_id=".$empresa.") AND (venta_fecha BETWEEN '$busqueda' AND '$fechaF') ORDER BY servicios.venta_id DESC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(venta_id) FROM servicios WHERE servicios.company_id=".$empresa." AND venta_fecha";

			}else{

				$consulta_datos="SELECT $campos_tablas FROM servicios INNER JOIN cliente ON servicios.cliente_id=cliente.cliente_id INNER JOIN usuario ON servicios.usuario_id=usuario.usuario_id WHERE servicios.company_id=".$empresa." ORDER BY servicios.venta_id DESC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(venta_id) FROM servicios WHERE servicios.company_id=".$empresa;
			...
			$tabla.='
				<div><i class="bi bi-bag-check-fill"></i> &nbsp;Servicios Realizados en este periodo: <strong>'.$total.'</strong><div>
			...
			if($total>=1 && $pagina<=$numeroPaginas){
				...
				foreach($datos as $rows){
					$tabla.='
						<tr class="has-text-centered" >
							...
							<td>'.MONEDA_SIMBOLO.number_format($rows['venta_total']).' '.MONEDA_NOMBRE.'</td>
			                <td>

			                	<button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_invoice(\''.APP_URL.'app/pdf/invoiceS.php?code='.$rows['venta_codigo'].'\')" title="Imprimir factura de Servicio Nro. '.$rows['venta_id'].'" >
	                                <i class="fas fa-file-invoice-dollar fa-fw"></i>
	                            </button>

                                <button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_ticket(\''.APP_URL.'app/pdf/ticketS.php?code='.$rows['venta_codigo'].'\')" title="Imprimir ticket de Servicio Nro. '.$rows['venta_id'].'" >
                                    <i class="fas fa-receipt fa-fw"></i>
                                </button>

			                    <a href="'.APP_URL.'servicesDetail/'.$rows['venta_codigo'].'/" class="button is-link is-rounded is-small" title="Informacion de Servicio Nro. '.$rows['venta_id'].'" >
			                    	<i class="fas fa-shopping-bag fa-fw"></i>
			                    </a>

			                	<form class="FormularioAjax is-inline-block" action="'.APP_URL.'app/ajax/serviceAjax.php" method="POST" autocomplete="off" >
				...
				if($total>=1){
					...
					$tabla.='
						<tr class="has-text-centered" >
							<td colspan="7">
								No hay Servicios Realizados registrados en esta fecha en el sistema
							</td>
						</tr>
					';
				}
			...
			if($total>0 && $pagina<=$numeroPaginas){
				$tabla.='<p class="has-text-right">Mostrando Servicios realizados <strong>'.$pag_inicio.'</strong> al <strong>'.$pag_final.'</strong> de un <strong>total de '.$total.'</strong></p>';
		...
		public function listarVentaControlador($pagina,$registros,$url,$busqueda){
			...
			if(isset($busqueda) && $busqueda!=""){
				...
				$consulta_datos="SELECT $campos_tablas FROM servicios INNER JOIN cliente ON servicios.cliente_id=cliente.cliente_id INNER JOIN usuario ON servicios.usuario_id=usuario.usuario_id WHERE (servicios.company_id=".$empresa.") AND (servicios.venta_codigo='$busqueda') ORDER BY servicios.venta_id DESC LIMIT $inicio,$registros";

				$consulta_total="SELECT COUNT(venta_id) FROM servicios WHERE (servicios.company_id=".$empresa.") AND (servicios.venta_codigo='$busqueda')";
		    ...
		    if($total>=1 && $pagina<=$numeroPaginas){
					...
					$tabla.='
						<tr class="has-text-centered" >
			                ...
			                <td>
			                	...
			                	<button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_invoiceS(\''.APP_URL.'app/pdf/invoiceS.php?code='.$rows['venta_codigo'].'\')" title="Imprimir factura Nro. '.$rows['venta_id'].'" >
	                                <i class="fas fa-file-invoice-dollar fa-fw"></i>
	                            </button>

                                <button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="print_ticketS(\''.APP_URL.'app/pdf/ticketS.php?code='.$rows['venta_codigo'].'\')" title="Imprimir ticket Nro. '.$rows['venta_id'].'" >
                                    <i class="fas fa-receipt fa-fw"></i>
                                </button>

			                    <a href="'.APP_URL.'servicesDetail/'.$rows['venta_codigo'].'/" class="button is-link is-rounded is-small" title="Informacion de venta Nro. '.$rows['venta_id'].'" >
			                    	<i class="fas fa-shopping-bag fa-fw"></i>
			                    </a>
				...
				if($total>=1){
					...
					$tabla.='
						<tr class="has-text-centered" >
			                <td colspan="7">
			                    😒 No hay registros de servicios realizados en el sistema
			                </td>
			            </tr>
					';
				}
		...
		public function eliminarVentaControlador(){
			...
			# Verificando venta #
		    $datos=$this->ejecutarConsulta("SELECT * FROM servicios WHERE company_id=".$empresa." AND venta_id='$id'");
		    if($datos->rowCount()<=0){
		        $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos encontrado la venta del servicio en el sistema",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
		    ...
		    $check_detalle_venta=$this->ejecutarConsulta("SELECT venta_detalle_id FROM servicios_detalle WHERE company_id=".$empresa." AND venta_codigo='".$datos['venta_codigo']."'");
		    $check_detalle_venta=$check_detalle_venta->rowCount();

		    if($check_detalle_venta>0){

		        $eliminarVentaDetalle=$this->eliminarRegistro("servicios_detalle","venta_codigo",$datos['venta_codigo']);
		    ...
		    $eliminarVenta=$this->eliminarRegistro("servicios","venta_id",$id);
		    ...
		    if($eliminarVenta->rowCount()==1){
		        ...
		        $alerta=[
					...
					"texto"=>"La venta del servicio ha sido eliminada del sistema correctamente",
					"icono"=>"success"
				];
